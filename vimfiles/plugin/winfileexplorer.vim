...
" Last Change: Fri Sep 05 07:00 PM 2003 
" Version: 2.5
...
function! WinFileExplorerAddToFavs(dirName)
	if s:favDirs !~ "\\(^\\|\n\\)".a:dirName."\\(\n\\|$\\)"
...
function! s:AddHeader()
	...
	let @f=@f."\"= ".s:ShortenPath(b:completePath)."\n"
	put! f
	let @f=save_f
endfunction

function! s:ShortenPath(fullPath)

    let sandboxPat = '.*/sandbox/savadhan/\w\+/matlab/'
    if a:fullPath =~ sandboxPat
        let prefix = matchstr(a:fullPath, sandboxPat)
        let suffix = a:fullPath[len(prefix):]

        let suffix1 = matchstr(suffix, '.*/\ze\w\+')
        let suffix1Short = substitute(suffix1, '\(\w\w\w\)\w\+', '\1', 'g')
        let suffix2 = suffix[len(suffix1):]

        return suffix1Short.suffix2.' ('.a:fullPath.')'
    else
        let prefix = matchstr(a:fullPath, '.*/\ze\w\+')
        let suffix = a:fullPath[len(prefix):]

        return suffix.' ('.a:fullPath.')'
    endif
...
function! s:PrintFavDirs()
	...
	call search('^"=')
	silent! put!=favDirs
	silent! g/^+ /s/^+ \(.*\)/\='+ '.s:ShortenPath(submatch(1))/
	call s:CleanUpHistory()
	call s:CleanUpHistory()
	setlocal nomodifiable nomodified
endfunction
