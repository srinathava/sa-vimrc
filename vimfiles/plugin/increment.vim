...
function! IncrementColumn(pad, ...)
	...
	" little trick to test, if there is c1 = c2 = c1v = c2v = 0
	" patch by Lubomir Host. exit if no column is selected.
	if c1 + c2 + c1v + c2v == 0 
		echo "No column choosen. Please use Ctrl-V to select area."
		return
	endif
	let clen = c2v - c1v
    ...
    if presNum =~ '\s*\d\+\s*'
        " remove any indentation which might have still remained.
        exec 'let presNum = '.presNum 
		let method = 'num'
		let pad = a:pad
	else
		let method = 'alpha'
	...
	if method == 'num'
		...
		while (r <= r2)
			...
			let prespart = strpart(getline('.'), cnow-1, clen+1)
			if prespart !~ '^[0-9]\+$'
				normal! j
				let r = r + 1
				continue
	...
	while (r <= r2)
		...
		let prespart = strpart(getline('.'), cnow-1, clen+1)
		if (method == 'num' && prespart !~ '\d\+') ||
			\ (method == 'alpha' && prespart !~ '\a')
			if pad
				call PrintError('indenting with padspace here, strlen = '.strlen(padspace))
				let extraspace = substitute(presNum, '.', ' ', 'g')
				let extraspace = substitute(extraspace, '.$', '', '')
				call setline('.', linebef.extraspace.padspace.prespart.lineaft)
			end
			normal! j
			let r = r + 1
			continue
		...
		if method == 'num'
			...
			let presNum = nr2char(char2nr(presNum) + 1)
		endif
